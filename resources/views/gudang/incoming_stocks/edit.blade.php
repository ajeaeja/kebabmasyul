    ...
    <div class="card-custom">
        ...
        <div class="card-body p-4">
            <form action="{{ route('incoming-stocks.update', $incomingStock->id) }}" method="POST">
                ...
                @if(!Auth::user()->isOwner() && !($incomingStock->incoming_date && \Carbon\Carbon::parse($incomingStock->incoming_date)->diffInHours(now()) < 24))
                    <div class="card bg-warning bg-opacity-10 border border-warning border-opacity-25 rounded-3 p-3 mb-4">
                        ...
                        <textarea class="form-control bg-white @error('edit_reason') is-invalid @enderror" id="edit_reason" name="edit_reason" rows="2" placeholder="Contoh: Koreksi nominal salah input..." required>{{ old('edit_reason') }}</textarea>
                        @error('edit_reason')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted d-block mt-1">Pengeditan data memerlukan persetujuan Owner. Data asli tidak akan berubah sebelum disetujui.</small>
                    </div>
                @endif
